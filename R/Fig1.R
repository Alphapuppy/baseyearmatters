...
Agprod1_1 <-  Agprod %>%
  ...
  left_join(Detailedland %>% filter(para %in% scen1, crop %in% crop10) %>%
              group_by(para,region, crop, year) %>%summarise(area = sum(area)/10)%>%  ungroup())%>% #change unit to Mha
...
Fig1_1 %>% filter(year == 2030, para == "Base2015", variable == "price")
Fig1_1 %>% filter(year == 2030, para == "Base2010", variable == "price")
